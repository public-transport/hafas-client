...
const request = async (ctx, userAgent, reqData) => {
	...
	const errProps = {
		...
		request: req.body, // todo: commit as bugfix
		url: url,
	}
	...
	if (body.err) errProps.hafasErrorCode = body.err
	if (body.errTxt) errProps.hafasErrorMessage = body.errTxt
	if (body.id) errProps.responseId = body.id

	if (body.err && body.err !== 'OK') {
		const err = new Error(body.errTxt || body.err)
		Object.assign(err, errProps)
		if (body.err in byErrorCode) {
			Object.assign(err, byErrorCode[body.err])
		}
		throw err
	}
	...
	if (body.svcResL[0].err !== 'OK') {
		const err = new Error(body.svcResL[0].errTxt || body.svcResL[0].err)
		Object.assign(err, errProps)
		if (body.svcResL[0].err in byErrorCode) {
			Object.assign(err, byErrorCode[body.svcResL[0].err])
		}
		throw err
	}
